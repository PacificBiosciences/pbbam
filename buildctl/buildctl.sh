...
set_globals() {
    ...
    g_cmake_rootdir=$(readlink -f "${g_progdir}/../../../../../prebuilt.tmpout/cmake/cmake_2.8.12.2-0ubuntu3")
    g_gcc_rootdir=$(readlink -f "${g_progdir}/../../../../../prebuilt.out/3.x/gcc/gcc-4.8.4/libc-2.5")

    g_cmake_cmd='
        PATH="${g_gcc_rootdir}/bin:$PATH"
        LD_LIBRARY_PATH="${g_gcc_rootdir}/lib/x86_64-linux-gnu:${g_gcc_rootdir}/lib:${g_cmake_rootdir}/lib/x86_64-linux-gnu:${g_cmake_rootdir}/usr/lib/x86_64-linux-gnu"
	CXX="${g_gcc_rootdir}/bin/g++"
	C="${g_gcc_rootdir}/bin/gcc"
	    ${g_cmake_rootdir}/usr/bin/cmake
    '
    g_cmake_cmd=$(echo $g_cmake_cmd)

    g_make_cmd='
        PATH="${g_gcc_rootdir}/bin:$PATH"
        LD_LIBRARY_PATH="${g_gcc_rootdir}/lib/x86_64-linux-gnu:${g_gcc_rootdir}/lib:${g_cmake_rootdir}/lib/x86_64-linux-gnu:${g_cmake_rootdir}/usr/lib/x86_64-linux-gnu"
            ${g_make_rootdir}/bin/make
    '
...
clean() {
    ...
    if [[ -d "${g_outdir}/build/Makefile" ]]; then
	pushd "${g_outdir}/build" > /dev/null
...
build() {
    ...
    ln -s "../../third-party/htslib/${g_outdir_name}/install-build" "${g_outdir}/deplinks/htslib"
    ...
    # build
    mkdir -p "${g_outdir}/build"

    pushd "${g_outdir}/build" > /dev/null
    eval "$g_cmake_cmd -DPacBioBAM_build_chef=ON -DPacBioBAM_build_docs=OFF -DPacBioBAM_build_tests=OFF ../.."
    eval "$g_make_cmd" ${1+"$@"}
    popd > /dev/null
}
install_build() {
    ...
    cp -a "${g_outdir}/lib/lib${g_name}.a"  "$g_installbuild_dir/lib"

    # install includes
    mkdir -p "$g_installbuild_dir/include"        
    for i in pbbam ; do
	mkdir -p "$g_installbuild_dir/include/$i"
	cp -a "${g_progdir}/../include/$i"/*.h "$g_installbuild_dir/include/$i"
    done
