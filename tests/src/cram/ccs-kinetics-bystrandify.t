...
ConsensusReadSet XML input

  $ grep -c "<pbds:TotalLength>44</pbds:TotalLength>" "${TESTDIR}"/../../data/ccs-kinetics-bystrandify/ccs-kinetics-bystrandify-mock-input.consensusreadset.xml
  1
  $ grep -c "<pbds:NumRecords>4</pbds:NumRecords>" "${TESTDIR}"/../../data/ccs-kinetics-bystrandify/ccs-kinetics-bystrandify-mock-input.consensusreadset.xml
  1
  ...
  $ grep -c "xml-test-out.bam\"" "${CRAMTMP}"/xml-test-out.consensusreadset.xml
  1
  $ grep -c "xml-test-out.bam.pbi\"" "${CRAMTMP}"/xml-test-out.consensusreadset.xml
  1

  $ "${SAMTOOLS}" view -h "${CRAMTMP}"/xml-test-out.bam | grep -v '@PG	ID:samtools'
  @HD	VN:1.5	SO:unknown	pb:3.0.1
  @RG	ID:ab118ebd	PL:PACBIO	DS:READTYPE=CCS;Ipd:CodecV1=ip;PulseWidth:CodecV1=pw;BINDINGKIT=101-490-800;SEQUENCINGKIT=101-490-900;BASECALLERVERSION=5.0.0;FRAMERATEHZ=100.000000	PU:m64011_190228_190319	PM:SEQUELII	CM:S/P3-C1/5.0-8M
  @PG\tID:ccs-kinetics-bystrandify-*\tPN:ccs-kinetics-bystrandify\tVN:* (glob)
  m64011_190228_190319/1/ccs/fwd	4	*	0	255	*	*	0	0	ACGTACGTACG	aRWQRRRPWRA	cx:i:3	np:i:2	ip:B:C,1,5,10,20,30,40,100,110,150,200,250	pw:B:C,2,6,11,21,31,41,101,111,151,201,251	zm:i:1	sn:B:f,13.4413,20.9895,4.68673,8.39738	rq:f:0.999867	RG:Z:ab118ebd
  ...
  $ grep -c "<pbds:TotalLength>64</pbds:TotalLength>" "${CRAMTMP}"/xml-test-out.consensusreadset.xml
  1
  $ grep -c "<pbds:NumRecords>8</pbds:NumRecords>" "${CRAMTMP}"/xml-test-out.consensusreadset.xml
  1
