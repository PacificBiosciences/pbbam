...
static void checkFilterRows(const PbiFilter& filter, const std::vector<std::size_t> expectedRows)
{
    if (expectedRows.empty()) {
        for (std::size_t row = 0; row < shared_index.NumReads(); ++row) {
            EXPECT_FALSE(filter.Accepts(shared_index, row));
        }
    } else {
        for (std::size_t row : expectedRows) {
            EXPECT_TRUE(filter.Accepts(shared_index, row));
        }
    }
}

static void checkFilterBarcodedRows(const PbiFilter& filter,
                                    const std::vector<std::size_t> expectedRows)
{
    if (expectedRows.empty()) {
        for (std::size_t row = 0; row < shared_barcoded_index.NumReads(); ++row) {
...
static void checkFilterInternals(const PbiFilter& filter,
                                 ...
                                 const std::size_t expectedNumChildren,
                                 const std::vector<std::size_t> expectedRows)
{
...
TEST(BAM_PbiFilter, SortsAndUniquesChildFilterResultsOk)
{
    ...
    PbiFilterTests::checkFilterRows(childFilter, std::vector<std::size_t>{2, 7, 0, 3, 4, 1, 8});
    PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0, 1, 2, 3, 4, 7, 8});
}
...
TEST(BAM_PbiFilter, can_filter_union_of_child_filters)
{
    ...
    PbiFilterTests::checkFilterInternals(u, PbiFilter::UNION, 2,
                                         std::vector<std::size_t>{0, 1, 2, 3});
}
{  // move
...
PbiFilterTests::checkFilterInternals(u, PbiFilter::UNION, 2, std::vector<std::size_t>{0, 1, 2, 3});
}
{  // move
    const auto u =
...
{  // 2-child union, results sorted & unique-d by PbiFilter
    ...
    PbiFilterTests::checkFilterRows(child1, std::vector<std::size_t>{2, 7, 0, 3, 4, 1, 8});
    PbiFilterTests::checkFilterRows(child2, std::vector<std::size_t>{3, 7, 5});
    PbiFilterTests::checkFilterRows(u, std::vector<std::size_t>{0, 1, 2, 3, 4, 5, 7, 8});
}
}
...
{  // 2-child intersect, sorted & unique-d by PbiFilter
    ...
    PbiFilterTests::checkFilterRows(child1, std::vector<std::size_t>{2, 7, 0, 3, 4, 1, 8});
    PbiFilterTests::checkFilterRows(child2, std::vector<std::size_t>{3, 7, 5});
    PbiFilterTests::checkFilterRows(i, std::vector<std::size_t>{3, 7});
}
}
...
{
    {
        ...
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{1});
    }
    {
        const auto filter = PbiFilter{PbiAlignedEndFilter{4055, Compare::NOT_EQUAL}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0, 2, 3});
    }
    {
        const auto filter = PbiFilter{PbiAlignedEndFilter{4000, Compare::LESS_THAN}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0});
    }
    {
        const auto filter = PbiFilter{PbiAlignedEndFilter{5560, Compare::GREATER_THAN}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{3});
    }
    {
        const auto filter = PbiFilter{PbiAlignedEndFilter{5560, Compare::GREATER_THAN_EQUAL}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{2, 3});
    }
...
{
    {
        ...
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{1, 2, 3});
    }
    {
        const auto filter = PbiFilter{PbiAlignedLengthFilter{1000, Compare::GREATER_THAN_EQUAL}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{1, 2});
    }
}
...
{
    {
        ...
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0, 1});
    }
    {
...
{
    {
        ...
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0, 2});
    }
    {
        const auto filter = PbiFilter{PbiAlignedStrandFilter{Data::Strand::REVERSE}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{1, 3});
    }
    {
...
{
    {
        ...
        PbiFilterTests::checkFilterBarcodedRows(filter, std::vector<std::size_t>{1, 3});
    }
    {
    ...
    }
    {
        ...
        PbiFilterTests::checkFilterBarcodedRows(filter, std::vector<std::size_t>{0});
    }
}
...
TEST(BAM_PbiFilter, can_filter_on_barcode_forward)
{
    ...
    }
    {
        ...
        PbiFilterTests::checkFilterBarcodedRows(filter, std::vector<std::size_t>{});
    }
    {
        const auto filter = PbiFilter{PbiBarcodeForwardFilter{{0, 256}}};
        PbiFilterTests::checkFilterBarcodedRows(filter, std::vector<std::size_t>{0, 2});
    }
    {
        //blacklist
...
TEST(BAM_PbiFilter, can_filter_on_movie_name)
{
...

    {
        ...
        const auto expectedRows = std::vector<std::size_t>{0, 1, 2, 3};
        for (std::size_t row : expectedRows) {
    ...
    }
    {
        ...
        const auto expectedRows = std::vector<std::size_t>{};
        for (std::size_t row : expectedRows) {
            EXPECT_TRUE(filter.Accepts(index, row));
        }
    }
    ...
    }
    {
        ...
        for (std::size_t row = 0; row < index.NumReads(); ++row) {
...
TEST(BAM_PbiFilter, can_filter_on_num_deleted_bases)
{
...

    {
        ...
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0, 3});
    }
    {
        const auto filter = PbiFilter{PbiNumDeletedBasesFilter{45, Compare::EQUAL}};
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{2});
    }
}
...
TEST(BAM_PbiFilter, can_filter_on_num_subreads)
{
    ...
    {  // >= 3
        ...
        const std::vector<std::size_t> expectedRows{0,  1,  2,  5,  6,  7,  8, 9,
    ...
    {  // < 3
        ...
        const std::vector<std::size_t> expectedRows{3, 4, 10, 18, 19, 20};
        for (std::size_t row : expectedRows) {
            EXPECT_TRUE(filter.Accepts(index, row));
        }
    }
    {  // == 1
        ...
        const std::vector<std::size_t> expectedRows{10, 20};
        for (std::size_t row : expectedRows) {
            EXPECT_TRUE(filter.Accepts(index, row));
        }
    }
...
TEST(BAM_PbiFilter, can_filter_on_read_group)
{
    {  // numeric ID
        ...
        PbiFilterTests::checkFilterRows(filter2, std::vector<std::size_t>{});
    }
    {  // string ID
        ...
        PbiFilterTests::checkFilterRows(filter2, std::vector<std::size_t>{0, 1, 2, 3});
    }
...
TEST(BAM_PbiFilter, can_filter_on_reference_end)
{
    ...
    }
    {
        ...
        PbiFilterTests::checkFilterRows(filter, std::vector<std::size_t>{0, 1, 3});
    }
}
...
TEST(BAM_PbiFilter, can_load_from_dataset)
{
    ...
    for (std::size_t i = 0; i < PbiFilterTests::shared_index.NumReads(); ++i) {
...
TEST(BAM_PbiFilter, can_load_from_dataset_with_barcode_list)
{
    auto runner = [](const Property& property, const PbiFilter& expectedFilter,
                     const std::vector<std::size_t>& expectedResults) {
        auto filter = Filter{};
    ...
    runner(Property{"bc", "[18,18]", "=="}, PbiBarcodesFilter{{18, 18}, Compare::EQUAL},
           std::vector<std::size_t>{});  // none share forward & reverse
...
TEST(BAM_PbiFilter, can_load_from_dataset_with_local_context)
{
    {  // no adapters or barcodes
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterBarcodedRows(expectedFilter, std::vector<std::size_t>{0});
        PbiFilterTests::checkFilterBarcodedRows(generatedFilter, std::vector<std::size_t>{0});
    }
    {  // any adapters or barcodes
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{1, 2, 3});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{1, 2, 3});
    }
    {  // contains adapter_before
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{1, 3});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{1, 3});
    }
    {  // contains adapter_before
    ...
    {  // contains adapter_after
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{2, 3});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{2, 3});
    }
    ...
    {  // contains adapter_before or adapter_after, but not both
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{1, 2});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{1, 2});
    }
    {  // contains adapter_before or adapter_after
    ...
    {  // adapter_before and adapter_after
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{3});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{3});
    }
    {  // adapter_before, but no adapter_after
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{1});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{1});
    }
    {  // contains no adapter_before
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{0, 2});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{0, 2});
    }
    {  // contains no adapter_before or adapter_after
        ...
        const auto generatedFilter = PbiFilter::FromDataSet(dataset);
        PbiFilterTests::checkFilterRows(expectedFilter, std::vector<std::size_t>{0});
        PbiFilterTests::checkFilterRows(generatedFilter, std::vector<std::size_t>{0});
    }
    {  // contains no adapter_before or adapter_after
